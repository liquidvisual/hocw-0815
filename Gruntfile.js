...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    watch: {
      ...
      jekyll: {
        files: [
          '<%= yeoman.app %>/**/*.{html,yml,md,mkd,markdown,txt}',
          '!<%= yeoman.app %>/_bower_components/**/*'],
        tasks: ['jekyll:server']
      },
      settings: { // watch for settings - since they contain breakpoints - communicating to sass
        files: ['<%= yeoman.app %>/_data/settings.yml'],
        tasks: ['jekyll:server', 'sass:server']
      }
    },
    //-----------------------------------------------------
    // BROWSER SYNC
  ...
  grunt.registerTask('build', [
    ...
    //'pixrem',
    'cdnify',
    // 'clean:utility', // left over by liquid vars
    //'htmlmin'
    ]);
